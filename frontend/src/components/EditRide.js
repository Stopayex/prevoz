...
function EditRide() {
    ...
    const edit = () => {
        if (from === "" || to === "") {setInfo("All fields are required");return;}
        api.put("rides/" + id, {from: from, to: to, date: date, time: time, seats: seats, price: price, car: car, phoneNumber: phoneNumber, notes: notes,}).then(r => {
            setIsSuccess(true);
            setInfo("Success");
